...
class CLIPTTA(nn.Module):
    ...
    def get_image_pred_local(self, model, return_full=False, return_logit=False):
        ...
        cancidate = indice[:self.args.local_top]
        # cancidate = indice[:, :10]
        # if return_full:
        ...
        # ipdb.set_trace()
        # attented_feat: torch.Size([1, 1, 1024])
        # adaptive_image_feat: torch.Size([1, 102, 1, 1024])
    ...
    def get_image_pred_fewshot_local(self, model, return_full=False, return_logit=False):
        ...
        cancidate = indice[:, :self.args.local_top]
        if return_full:
            img_feat = model.image_features_local ## extract the importance feat.
        else:
            cancidate = cancidate[:1]
            img_feat = model.image_features_local[:1, :, :]  # 1*L*1024

        text_features_cancidate = model.text_feat[cancidate]  ## batch* can*1024
        ...
        attented_feat = attented_feat / attented_feat.norm(dim=-1, keepdim=True)  ## 和memorized local image feature 算prediction.
        memorized_image_feat = model.fixed_local_feat  ## 200*11*1024, only the fixed local feat
        if self.learnable_mapping == 'bias':
            memorized_image_feat = memorized_image_feat + self.local_bias.unsqueeze(1)  ## class*shot*1024
            attented_feat = attented_feat + self.local_bias.mean(0, keepdim=True)  ## N*1024
        elif self.learnable_mapping == 'affine':
            memorized_image_feat = memorized_image_feat + memorized_image_feat @ self.local_affine
            attented_feat = attented_feat + attented_feat @ self.local_affine
        elif self.learnable_mapping == 'all':
            memorized_image_feat = memorized_image_feat + memorized_image_feat @ self.local_affine + self.local_bias.unsqueeze(1)  ## class*shot*1024
            attented_feat = attented_feat + attented_feat @ self.local_affine + self.local_bias.mean(0, keepdim=True)  ## N*1024
        else:
            raise NotImplementedError
        ...
        logit_scale = model.logit_scale.exp()
        # attented_feat: torch.Size([1, 1, 900])
        # adaptive_image_feat: torch.Size([1, 102, 1, 900])
        logits = logit_scale * (attented_feat.unsqueeze(1)[..., self.args.indice] * adaptive_image_feat[..., self.args.indice]).sum(-1)  ## batch*class num*candidate
        logits = logits.transpose(1,2)
            ...
            for lambda_ape in lambda_ape_list:
                for lr in lr_list:
                    for epoch in epoch_list:
                        ...
                        print_log("lr: {}, epoch:{}, num_important_channel{}, lambda_ape: {}, best acc{:.2f} \n".format(lr, epoch, num_important_channel, lambda_ape, results_temp[5]), end="	")
                        if results_temp[5] > best_acc:
                            results[set_id] = results_temp
                            best_acc = results_temp[5]
...
def direct_inference(val_loader, model, args):
    ...
    if weight_search:
        ...
        if args.mute_dynamic:
            beta2_list = [0]   ## weight predictions of dynamic memories as zero.
            beta3_list = [0]
        if args.mute_static:
            beta4_list = [0]   ## weight predictions of dynamic memories as zero.
            beta5_list = [0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mute_dynamic', action='store_true', default=False, help="remove dynamic results.")
    parser.add_argument('--mute_static', action='store_true', default=False, help="remove dynamic results.")
