...
class CLIPTTA(nn.Module):
            ...
            if value.item() > 0.0:
                ...
                if model.image_feature_count[pseudo_label] == model.memory_size:
                    ...
                    if (current_instance_entropy < model.image_entropy_mem[pseudo_label]).sum() == 0:
                        ...
                        model.image_prediction_mem[pseudo_label][to_replace_indice] = mean_prob[0]
                        model.local_prediction_mem[pseudo_label][to_replace_indice] = mean_prob[0]
                        model.image_entropy_mem[pseudo_label][to_replace_indice] = current_instance_entropy
                else:
                    ...
                    model.image_prediction_mem[pseudo_label][model.image_feature_count[pseudo_label, 0].item()] = mean_prob[0]
                    model.local_prediction_mem[pseudo_label][model.image_feature_count[pseudo_label, 0].item()] = mean_prob[0]
                    model.image_entropy_mem[pseudo_label][model.image_feature_count[pseudo_label, 0].item()] = current_instance_entropy
                    model.image_feature_count[pseudo_label] += 1
                    model.local_feature_count[pseudo_label] += 1
        else:
    ...
    def get_image_pred(self, model, return_full=False, return_logit=False):
        ...
        image_classifier = 'prob_weighted'  ## category_center | weighted_center | similarity_weighted | prob_weighted
        ...
        ## [image_feat_memory  init text features]
        memorized_image_feat = torch.cat((model.image_feature_memory, model.fixed_global_feat_vanilla), dim=1)  ## 200*11*1024
        memorized_image_label = torch.cat((model.image_prediction_mem, model.fixed_global_label_vanilla), dim=1)  ## N * L * N
        num_expand_mem = model.memory_size + model.fixed_global_label_vanilla.shape[1]
        memorized_image_count = model.image_feature_count + model.fixed_global_label_vanilla.shape[1]  ## N * 1
        if image_classifier == 'weighted_center':
            ...
            return logits.softmax(dim=1)
        elif image_classifier == 'prob_weighted':
            ## calculate the cos similarity betweeen image feature and memory feature, and then weighted the memorized probability.
            ###################### 有一些memory 是空的，现在却往里面塞了一个self.global_bias， 这不合理，还要把它继续置空。
            with torch.no_grad():
                if self.learnable_mapping == 'bias':
                    ...
                    memorized_image_feat_K = memorized_image_feat + memorized_image_feat @ self.global_affine + self.global_bias.unsqueeze(
                        1)  ## class*shot*1024
                    img_feat_mappling = img_feat + img_feat @ self.global_affine + self.global_bias.mean(0,
                                                                                                         keepdim=True)  ## N*1024
                else:
                    raise NotImplementedError
                memorized_image_feat_K = memorized_image_feat_K / memorized_image_feat_K.norm(dim=-1, keepdim=True)
            ...
            similarity_matrix = (img_feat_mappling * memorized_image_feat_K).sum(-1)  ## 200*11  理应-1~1, 但是实际都是 0.1~0.2， 可能和训练方式有关
            # similarity_matrix = torch.exp(-100 * (-similarity_matrix + 1))  ## N*(L+1), 200*51
            similarity_matrix = torch.exp(-self.beta * (-similarity_matrix + 1))  ## N*(L+1), 200*51
            ####### 这里要修改吗？ 这里实际上还是类似对所有的样本做加权组合？ 只是权重是adaptative 的； 但是目前的权重基本都是相同的差异性没有拉开
            ########### image-image 的相似度是比较高的，0.5-0.6 左右，所以image 贡献了比较多的权重。
            ## 但是最开始image 可能是不准确的； 所以最好开始给 text 较大的权重:发现结果正好相反，开始要给text 比较小的权重，否则结果很差
            ######## 对similarity matrix 的shape 进行重新refine
            ## 根据同image feat similarity 进行加权, 得到new classifier.
            pred_sum = (memorized_image_label * similarity_matrix.unsqueeze(-1)).sum(0).sum(0)  ## N
            pred_sum = pred_sum * (num_expand_mem / memorized_image_count[:,0])  ## N
            output = pred_sum / pred_sum.sum()
            model.test_global_pred = output ## N
            return output.unsqueeze(0)  ## = L1 norm.
        else:
            raise NotImplementedError
    ...
    def get_image_pred_local(self, model, return_full=False, return_logit=False):
        ...
        image_classifier = 'prob_weighted'
        ...
        ####### local feature with cos scale 100.
        ...
        memorized_image_label = torch.cat((model.local_prediction_mem, model.fixed_local_label_vanilla), dim=1)  ## N * L * N
        num_expand_mem = model.memory_size + model.fixed_local_label_vanilla.shape[1]
        memorized_image_count = model.local_feature_count + model.fixed_local_label_vanilla.shape[1]  ## N * 1
        ...
        similarity_matrix = torch.exp(-self.beta * (-similarity_matrix + 1))  ## 1*200*10*11 (view/batch*class*can*memory)  make it more sharp.

        if image_classifier == 'similarity_weighted':
            ...
            ##### 直接对label 进行加权； 但是这样会得到 candidiate number pseudo label. 然后从里面挑出一个。
            # ipdb.set_trace()
            pred_sum = (similarity_matrix[0] @ memorized_image_label).sum(0)  ## can*N, 5*102
            pred_sum = pred_sum * (num_expand_mem / memorized_image_count[:, 0])  ## can * N
            output = pred_sum / pred_sum.sum(1, keepdim=True)
            entropy = -(output * torch.log(output + 1e-8)).sum(-1)   ## 10个里挑一个。
            _, indice_min = entropy.min(0)  ### 这里要判断，这个特征是否是
            return  output[indice_min].unsqueeze(0)  ## 1*102
    ...
    def get_image_pred_fewshot_local(self, model, return_full=False, return_logit=False):
        ...
        #################### ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
        weight_prob = (cos_sim * 100).softmax(1)  ## L softmax.,  batch*L*can, 分类用的softmax, 可能需要调参。
...
def init_image_memory(train_loader, model, args):
    ...
    memorized_image_global_feat = [] ## N*[shot*aug]*d
    memorized_image_local_feat = []  ## N*[shot*aug]*d
    memorized_image_global_feat_vanilla = [] ## N*shot*d
    memorized_image_local_feat_vanilla = []  ## N*shot*d
    memorized_labels = []
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        memorized_image_global_feat[target].append(image_features_global) ## aug*d
        memorized_image_local_feat[target].append(attented_feat)   # aug * d
        memorized_image_global_feat_vanilla[target].append(image_features_global[:1]) ## aug*d
        memorized_image_local_feat_vanilla[target].append(attented_feat[:1])   # aug * d
        one_hot_target = torch.zeros(1, model.n_cls).to(target.device)
        one_hot_target[0, target] = 1
        memorized_labels[target].append(one_hot_target)   ## 1 * d, turn it to one hot labels.

    for i in range(model.n_cls):
        memorized_image_global_feat[i] = torch.cat(memorized_image_global_feat[i], dim=0).unsqueeze(0) ## 1*augshot*d
        memorized_image_local_feat[i] = torch.cat(memorized_image_local_feat[i], dim=0).unsqueeze(0)
        memorized_image_global_feat_vanilla[i] = torch.cat(memorized_image_global_feat_vanilla[i], dim=0).unsqueeze(0) ## 1*shot*d
    ...
    memorized_image_global_feat = torch.cat(memorized_image_global_feat, dim=0) ## n*[shot*aug]*d
    memorized_image_local_feat = torch.cat(memorized_image_local_feat, dim=0)
    memorized_image_global_feat_vanilla = torch.cat(memorized_image_global_feat_vanilla, dim=0) ## n*shot*d
    memorized_image_local_feat_vanilla = torch.cat(memorized_image_local_feat_vanilla, dim=0)
    memorized_labels = torch.cat(memorized_labels, dim=0) # n*shot*n
    ## n*shot*n (n=class number) ## 每一个类别内部加权求和； 再在类间求平均？这样不对。。类别间的排挤效果消失了。
    ## 如果在所有sample 上加权求和，由于不同类别样本数不同，导致每个类的先验不同了。。
    # 在所有样本上加权求和，得到求和之后的prob; 对求和之后的prob 做category sample 的reweighting (* 样本数量倒数)； 再做L1 norm.
    ...
    model.fixed_global_feat_vanilla = torch.cat((model.fixed_global_feat, memorized_image_global_feat_vanilla), dim=1)  ## N*1*d
    model.fixed_local_feat_vanilla = torch.cat((model.fixed_local_feat, memorized_image_local_feat_vanilla), dim=1)  ## N*1*d

    ###################### for static memory, with text feature and augmented image feat
    model.fixed_global_feat = torch.cat((model.fixed_global_feat, memorized_image_global_feat), dim=1)  ## N*1*d
    model.fixed_local_feat = torch.cat((model.fixed_local_feat, memorized_image_local_feat), dim=1)  ## N*1*d
    # ipdb.set_trace()
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if args.n_shot:
            with torch.no_grad():
                ...
                # cliptta.init_pred = (confidence_prediction.mean(0, keepdim=True) + confidence_prediction_fewshot_global.mean(0, keepdim=True)
                # + confidence_prediction_fewshot_local.mean(0, keepdim=True)) / 3
        ...
        ######## 原本是先update mem, 再 memory prediction; 但是我想把 global & local softlabel 存进memory 中；
        # 这样就导致需要先用local & global 做prediction, 再update mem.
