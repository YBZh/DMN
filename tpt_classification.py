...
def align_entropy_confused(outputs):
    logits = outputs - outputs.logsumexp(dim=-1, keepdim=True) # logits = outputs.log_softmax(dim=1) [N, 1000]
    avg_logits = logits.logsumexp(dim=0) - np.log(logits.shape[0]) # avg_logits = logits.mean(0) [1, 1000]
    min_real = torch.finfo(avg_logits.dtype).min
    avg_logits = torch.clamp(avg_logits, min=min_real)
    ...
    for j in range(args.tta_steps):
        with torch.cuda.amp.autocast():
            ...
            if selected_idx is not None:
                ...
                output, selected_idx, confused_output, confused_idx = select_confident_samples(output, args.selection_p)
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        ######### use it to decide the training or evaluation processes.
        for name, param in model.named_parameters():
            if not args.cocoop:
                if "prompt_learner" not in name:
                    param.requires_grad_(False)
        ...
        if not args.cocoop: # no need to reset cocoop because it's fixed
            if args.tta_steps > 0:
                with torch.no_grad():
                    model.reset()    #### 如果不reset 回去，那么结果会很快崩掉； 因为模型会比较快的偏离公平的prediction, 而偏向最开始的那几类！
        ...
        ######### turn all to False; for the use in the eval process.
        for name, param in model.named_parameters():
            if not args.cocoop:
                param.requires_grad_(False)
            else:
