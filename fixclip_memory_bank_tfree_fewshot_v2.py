######### compared to vanilla implementation, we use the labeled data to extract dynamic weight, instead of the tip implementation.
######### tip prompt + cupl
######### add augmented labeled data to memory.
...
class CLIPTTA(nn.Module):
    ...
    def get_image_pred(self, model):
        ...
        merged_image_feat = torch.cat((model.image_feature_memory, model.fixed_global_feat_vanilla), dim=1)  ## 200*11*1024
    ...
    def get_image_pred_local(self, model):
        ...
        merged_image_feat = torch.cat((model.local_feature_memory, model.fixed_local_feat_vanilla), dim=1)  ## 200*11*1024
    ...
    def get_image_pred_fewshot_local(self, model):
        ...
        merged_image_feat = model.fixed_local_feat  ## 200*11*1024, only the fixed local feat
        similarity_matrix = attented_feat.unsqueeze(1) @ merged_image_feat.transpose(1,2)  ## 1*200*10*11 (view*class*can*memory)
        # similarity_matrix: N*n_cls*can*shot
        ## fewshot_label:   class*shot*class   期望输出：N*can*class
        similarity_matrix = torch.exp(-self.beta * (-similarity_matrix + 1))  ## make it more sharp.
    ...
    def get_image_pred_fewshot_global(self, model):
        ...
        # merged_image_feat = torch.cat((model.image_feature_memory, model.text_features.unsqueeze(1)), dim=1) ## 200*11*1024
        ## fixed init text features.
        merged_image_feat = model.fixed_global_feat  ## 200*11*1024, few shot samples and text features.
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if args.n_shot:
            with torch.no_grad():
                ...
                # cliptta.init_pred = (confidence_prediction.mean(0) + confidence_prediction_fewshot_global.mean(0) + confidence_prediction_fewshot_local.mean(0)) / 3
