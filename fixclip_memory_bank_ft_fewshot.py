######### training 的时候用few shot 的样本训练一个 learnable QKV and FFN, 都用lora 的方式实现
...
class CLIPTTA(nn.Module):
    def __init__(self, beta=5.5, feat_dim=1024, class_num=1000, mapping='bias'):
        ...
        # self.lora_weight_K = nn.Parameter(torch.eye((feat_dim)))
        ...
        # self.lora_V = nn.Parameter(torch.zeros((feat_dim, feat_dim)))

        # self.lora_FFN = nn.Parameter(torch.zeros((feat_dim, feat_dim)))
...
def direct_inference(val_loader, model, args):
    ...
    pred_fewshot_local = []
    labels = []
    cliptta = CLIPTTA(beta=args.beta, feat_dim=feat_dim, class_num=class_num, mapping=args.mapping).cuda()
