...
class CLIPTTA(nn.Module):
    def __init__(self, args=None, beta=5.5, feat_dim=1024, class_num=1000, mapping='bias'):
        ...
        if args.shared_param:
            ...
            self.global_affine_key = self.global_affine
            self.global_affine_value = self.global_affine
            ...
            self.global_affine = nn.Parameter(torch.zeros((feat_dim, feat_dim)))
            self.global_affine_key = nn.Parameter(torch.zeros((feat_dim, feat_dim)))
            self.global_affine_value = nn.Parameter(torch.zeros((feat_dim, feat_dim)))
            self.global_bias = nn.Parameter(torch.zeros((class_num, feat_dim))) ## unknown use the category mean.
        ...
        if image_classifier == 'weighted_center':
            ...
            with torch.no_grad():
                if self.args.position == 'query':
                    ...
                    memorized_image_feat_K = memorized_image_feat + memorized_image_feat @ self.global_affine_key
                    ...
                    memorized_image_feat_V = memorized_image_feat + memorized_image_feat @ self.global_affine_value
            ...
            if self.args.position == 'output' or self.args.position == 'all' or self.args.position == 'vo':
    ...
    def forward(self, model):
        self.get_text_prediction(model)
        self.get_image_pred(model)
        self.get_image_pred_fewshot_global(model)
...
def direct_inference(val_loader, model, args):
    ...
    if args.n_shot and args.ft:
        ...
        for train_idx in range(epoch): ## for each epoch.
            ...
            # macs, params = profile(cliptta, model)
